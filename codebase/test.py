...
class Model(object):
    ...
    def create_transitions_and_states(self):
        ...
        BOB_subset = self.wafer_info_dummy.loc[self.wafer_info_dummy['BOB'] == True, 'TO']
        BOB_subset = list(set(BOB_subset))
        print(set(self.wafer_info.loc[self.wafer_info['BOB'] == True, 'CASE_ID']))
        #print(BOB_subset)
        WOW_subset = self.wafer_info_dummy.loc[self.wafer_info_dummy['WOW'] == True, 'FROM']
        WOW_subset = list(set(WOW_subset))
        print(set(self.wafer_info.loc[self.wafer_info['WOW'] == True, 'CASE_ID']))
        #print(WOW_subset)
        ...
        #print(states_group['wafer'][(states_group['states'].isin(BOB_subset)) & (-states_group['states'].isin(WOW_subset))])
        states_group['wafer'][(states_group['states'].isin(WOW_subset)) & (-states_group['states'].isin(BOB_subset))] = 'WOW'
        #print(states_group['wafer'][(states_group['states'].isin(WOW_subset)) & (-states_group['states'].isin(BOB_subset))])
        states_group['wafer'][(states_group['states'].isin(BOB_subset)) & (states_group['states'].isin(WOW_subset))] = 'B/W'
        #print(states_group['wafer'][(states_group['states'].isin(BOB_subset)) & (states_group['states'].isin(WOW_subset))])

        self.states = [tuple(x) for x in states_group[['states', 'chamber', 'wafer']].values]
        self.wafer_trans.to_csv('../result/wafer_trans.csv', sep = ',')
