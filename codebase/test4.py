...
wafer_info_dummy_equal_to = wafer_info.copy()

unique_from = list(set(wafer_info_dummy_equal_to['FROM']))
for _from in unique_from:
    tos = list(set(wafer_info_dummy_equal_to.loc[wafer_info_dummy_equal_to['FROM'] == _from]['TO']))
    tos_attr = wafer_group.loc[wafer_group['S_E_CHAMBER_ID'].isin(tos)]['LCresult']
    if checkEqual1(tos_attr):
        wafer_info_dummy_equal_to['FROM'][wafer_info_dummy_equal_to['FROM'] == _from] = _from[:8] + '_' + 'dummy'


wafer_trans = wafer_info_dummy_equal_to.groupby(['FROM', 'TO']).agg([np.count_nonzero, np.mean, np.std])
