...
class StatAnalyzer(object):
	...
	def produce_stat_analysis(self, alpha_LC = 0.05, alpha_NLC = 0.05):
		print("Linear_Contrasts begins, alpha_LC = {}, alpha_NLC = {}".format(alpha_LC, alpha_NLC))

		self.data.loc[:,'LC_p_value'] = 1.0
		self.data.loc[:, 'LCresult'] = 'DUMMY'

		activities = self.eventlog['ACTIVITY'].unique()

		for activity in activities:
		    #count +=1
		    #self.eventlog를 특정 STEP으로 filtering
		    filtered = self.eventlog.filter('ACTIVITY', activity)
		    #해당 step의 self.chamber_stat, filtered['RESOURCE'].unique()가 필요, 어떤 chamber가 있는지가 필요하므로
		    activity_resource_stat = self.data.loc[filtered['RESOURCE'].unique()]

		    #pass Normality test
		    if all(activity_resource_stat['Normality'] == True):
		        #ANOVA result: 유의미한 차이 없음
		        if (activity_resource_stat['KRUSKAL'] == True).any():
		            self.data.loc[activity_resource_stat.index, 'LCresult'] = 'DUMMY'
		        else:
		            #Linear Contrast 수행
		            sorted_resource_stat = self.data.loc[filtered['RESOURCE'].unique()].sort_values(by=['mean'],ascending=False)
		            len_sorted_resource_stat = len(sorted_resource_stat)
		            for index, row in sorted_resource_stat.iterrows():
		                p_value = self.Linear_Contrast(row, self.data.loc[filtered['RESOURCE'].unique()], activity)
		                self.data.loc[row.name, 'LC_p_value']  = p_value
		                #Linear Contrast 결과 유의미한 경우
		                if p_value < alpha_LC:
		                    self.data.loc[row.name, 'LCresult'] = 'BOB'
		                else:
		                    break
		            sorted_resource_stat = self.data.loc[filtered['RESOURCE'].unique()].sort_values(by=['mean'])
		            for index, row in sorted_resource_stat.iterrows():
		                p_value = self.Linear_Contrast(row, self.data.loc[filtered['RESOURCE'].unique()], activity)
		                self.data.loc[row.name, 'LC_p_value']  = p_value
		                if p_value < alpha_LC:
		                    self.data.loc[row.name, 'LCresult'] = 'WOW'
		                else:
		                    break
		    ...
		    #Non-parametric method
		        ...
		        if (activity_resource_stat['KRUSKAL'] == True).any():
		            ...
		            for index, row in sorted_resource_stat.iterrows():
		                count += 1
		                if count >= len_sorted_resource_stat/2:
		                    break

		                p_value = self.Nonparametric_Linear_Contrast(row, self.data.loc[filtered['RESOURCE'].unique()], activity)
		                self.data.loc[row.name, 'LC_p_value']  = p_value
		                #Linear Contrast 결과 유의미한 경우
		                if p_value < alpha_NLC:
		                    self.data.loc[row.name, 'LCresult'] = 'BOB'
		                else:
		                    break
		            #from the worst
		            sorted_resource_stat = self.data.loc[filtered['RESOURCE'].unique()].sort_values(by=['mean'])
		            #print(sorted_resource_stat)
		            count = 0
		            for index, row in sorted_resource_stat.iterrows():
		                count += 1
		                if count >= len_sorted_resource_stat/2:
